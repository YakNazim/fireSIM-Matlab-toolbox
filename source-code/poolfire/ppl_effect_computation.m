function [P_level1,P_level2, P_level3] = ppl_effect_computation(Fk , pos, pool_diameter , fit_R_f_heatflux  ,fit_heatflux_f_R,hfx)
...
x0 = dis - pool_diameter / 2 ; % distance between safe area & flame surface
t_eff = round( tr + ((x0 - ( pos - pool_diameter / 2 ) ) / u ) , 1);
if(t_eff < 0) 
    t_eff = 0.0001 ;
end
...
if (hfx == -1)
    hfx = round(return_heatflux(pos,fit_heatflux_f_R),1) ;
end
